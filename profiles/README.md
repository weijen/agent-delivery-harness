...
| `ci_signatures` | `PROFILE_CI_SIGNATURES` | Extended-regex grep tokens (the gate-command signatures) proving a project-CI workflow runs this surface's gates. Consumed by `scripts/lib/ci-coverage-lib.sh` for the preflight WARN and Pre-PR `ci-gate` (issue #129). This is a deliberately loose substring heuristic — a workflow that merely *mentions* a token (even in a comment) counts as covered; keep signatures specific enough to avoid false matches. |
