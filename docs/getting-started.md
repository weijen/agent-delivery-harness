...
| Retired source bundle | The Claude hook, guide, settings template and sensors formerly under `optional/runtime-adapters/` have been removed. Neither install profile distributes them; archive text and ownership history remain. |
...
Preflight, issue startup, PR publication, merge and closeout now live in `scripts/lifecycle/`, with their sensors
under `tests/scripts/lifecycle/`; `scripts/init.sh`, `scripts/start-issue.sh`,
`scripts/create-pr.sh`, `scripts/merge-pr.sh` and `scripts/finish-issue.sh`
...
The installer and language scaffolder now live in `scripts/install/`, with their sensors under
`tests/scripts/install/`, behind the stable `scripts/install-harness.sh` and
