...
後來 #305 把整個 runtime capture 層退役：既然「零筆 runtime span」已經是正常狀態，以 runtime span 為準的「dark run」判準也失去作用。sensor 改守 **semantic spine**：完整的 issue window（有 `worktree_create` 與 `finish` lifecycle span）若缺少 harness 自己發出的 agent span，才回報 `spine_incomplete`；runtime span 不再影響判定。#370 又移除 `feature_start` 的現行門檻，但保留舊 span 的相容性。權威說明見 `docs/evaluation/observability-and-trace-schema.md` 的 The Capture Retirement Boundary 一節。
