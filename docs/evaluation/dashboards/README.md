...
| Failure-detail log panel | Single-run drill-down | (deferred — `traces/log.jsonl`) | `log-schema.v1.json` (#219/#220) | `harness.issue` | **Deferred / unavailable — gated on #220**: the failure-detail log join (Tab 2 panel 4) is intentionally NOT shipped in this issue; it renders an explicit unavailable state, never an empty chart or fabricated data. |
