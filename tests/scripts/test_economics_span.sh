...
copy_fixture_scripts() {
  ...
  for s in lib/finish-lib.sh lib/economics-report-lib.sh lib/trace-lib.sh log-handback.sh trace/log-handback.sh check-trace-consistency.sh trace/check-trace-consistency.sh lib/issue-lib.sh; do
    [ -f "${ROOT}/scripts/${s}" ] \
...
run_economics_stamp() {
  ...
  (
    ...
    env -u COPILOT_AGENT_SESSION_ID PATH="$BIN" ISSUE_NUM="$issue" WORKTREE_DIR="" SCRIPT_DIR="${dir}/scripts" TRACE_ISSUE="$issue" \
      ...
      bash -c 'source scripts/lib/trace-lib.sh; source scripts/lib/finish-lib.sh; source scripts/lib/economics-report-lib.sh; trace_report_economics_stamp >/dev/null'
  )
}
...
LIB="${ROOT}/scripts/lib/economics-report-lib.sh"
...
assert_complete_contract "${ROOT}/scripts/lib/economics-report-lib.sh" "$COMPLETE"
...
JSONL
markdown="$(run_markdown "${ROOT}/scripts/lib/economics-report-lib.sh" "$MISSING")"
numeric="$(run_numeric "${ROOT}/scripts/lib/economics-report-lib.sh" "$MISSING")"
...
grep -Fx -- '- Review rounds: 0' <<< "$(run_markdown "${ROOT}/scripts/lib/economics-report-lib.sh" "$NONE")" >/dev/null \
  || fail "no review verdict spans must remain a measured zero"
grep -Fx -- 'harness.economics.review_rounds=0' <<< "$(run_numeric "${ROOT}/scripts/lib/economics-report-lib.sh" "$NONE")" >/dev/null \
...
sed 's/\\t\\($mode)//' "${ROOT}/scripts/lib/economics-report-lib.sh" > "$MUTATED"
if cmp -s "${ROOT}/scripts/lib/economics-report-lib.sh" "$MUTATED"; then
