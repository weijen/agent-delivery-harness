#!/usr/bin/env bash
# test_finish_issue_log_export.sh — regression sensor for the closeout
# best-effort LOG export (issue #220, feature log-export-finish-wiring).
#
# The direct sibling of test_finish_issue_trace_export.sh: it pins the LOG
# analogue of best_effort_trace_export. finish-issue.sh must, at closeout,
# attempt `scripts/log-export.sh <issue>` ONLY when configured (LOG_EXPORT_OTLP=1
# AND a non-empty APPLICATIONINSIGHTS_CONNECTION_STRING) — a best-effort step
# that mirrors the trace gate's opt-in-requires-BOTH-vars semantics:
#     1. unconfigured             → clean no-op, exporter NOT invoked.
#     2. LOG_EXPORT_OTLP=1 only    → still a no-op (the gate needs BOTH vars).
...
#   export never blocks the finish. The export reads the MAIN-checkout log
#   file (which survives worktree removal), so the exporter runs from the main
#   repo and its recorded call log lands at the main root, not in the removed
#   worktree.
#
# Fixture style mirrors test_finish_issue_trace_export.sh exactly: a temp MAIN
# repo, the worktree created via the REAL start-issue.sh SKIP_INIT=1, a pinned
# PATH bin (symlinked real coreutils/git/jq/etc.), and a fake gh (exit 1). A
# FAKE scripts/log-export.sh records its args to
# <main-root>/log-export-calls.log and exits per FAKE_EXPORT_EXIT, so the
# sensor observes whether — and how — finish-issue wired the exporter.
# finish-issue.sh resolves ${SCRIPT_DIR}/log-export.sh, so the fake in the temp
# repo scripts/ dir is what runs. FORCE=1 + a COMPLETE feature_list let the
# completion check pass so teardown proceeds.
#
# RED until finish-issue.sh calls best_effort_log_export at closeout: cases 3/4
# fail because the exporter is never invoked (no call log, no export warning).
...
for s in issue-lib.sh start-issue.sh finish-issue.sh finish-lib.sh check-feature-list.sh trace-lib.sh log-export.sh; do
  [ -f "${ROOT}/scripts/${s}" ] \
...
# A fake exporter: appends its args to <main-root>/log-export-calls.log and
# exits per FAKE_EXPORT_EXIT (default 0). ${BASH_SOURCE[0]} is the fake at
# <main-root>/scripts/log-export.sh, so ../log-export-calls.log is the
# main-root log that survives worktree removal.
write_fake_exporter() {
  cat > "$1" <<'SH'
#!/usr/bin/env bash
here="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
printf '%s\n' "$*" >> "${here}/../log-export-calls.log"
...
# the worktree tracking dir and a FAKE scripts/log-export.sh in the main repo.
make_export_fixture() {
...
[ ! -s "${R1}/log-export-calls.log" ] \
  || { cat "${R1}/log-export-calls.log"; fail "unconfigured no-op: exporter must NOT be invoked when LOG_EXPORT_OTLP is unset"; }

# ============================================================================
# 2. LOG_EXPORT_OTLP=1 but NO connection string → still a no-op (the gate
#    requires BOTH vars). Teardown happens, exporter NOT invoked.
# ============================================================================
R2="${TMP_DIR}/r41"
make_export_fixture "$R2" 41
{ (cd "$R2" && PATH="$BIN" FORCE=1 LOG_EXPORT_OTLP=1 ./scripts/finish-issue.sh 41 SLUG=fixture) \
...
[ ! -s "${R2}/log-export-calls.log" ] \
  || { cat "${R2}/log-export-calls.log"; fail "missing connection string: exporter must NOT be invoked without APPLICATIONINSIGHTS_CONNECTION_STRING (finish gates on BOTH vars)"; }
...
[ -s "${R3}/log-export-calls.log" ] \
  || fail "configured export: exporter MUST be invoked when both LOG_EXPORT_OTLP=1 and APPLICATIONINSIGHTS_CONNECTION_STRING are set (finish-issue.sh does not wire the closeout log export yet)"
grep -qw '42' "${R3}/log-export-calls.log" \
  || { cat "${R3}/log-export-calls.log"; fail "configured export: exporter must be called with the issue number (42) as an argument"; }
...
printf 'finish-issue closeout log-export contract honored\n'
