...
for sensor in tests/scripts/test_claude_adapter.sh tests/scripts/install/test_install_harness_claude.sh; do
...
for sensor in tests/scripts/test_adopter_smoke_workflow.sh tests/scripts/test_adopter_workflow_contract.sh \
  tests/scripts/install/test_install_harness_dev_profile.sh; do
  if [ -f "${ROOT}/${sensor}" ]; then
