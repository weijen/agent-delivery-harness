...
#   * Shared libraries, schema/contract authorities and scaffolding use the
#     same scoped mapping; real full-suite wrappers remain boundary-only.
#   * A declared sensor missing on disk → diagnostic and exit 2.
...
# 3. Shared changes must not widen the existing mapped set to a full suite.
cp "$OUT" "${TMP_DIR}/mapped"
for shared in scripts/lib/trace-lib.sh scripts/lib/finish-lib.sh \
  ...
  cmp -s "$OUT" "${TMP_DIR}/mapped" \
    || fail "changed ${shared} must preserve scoped selection (got: $(cat "$OUT"))"
done
...
# 5. Declarations are mandatory coverage, not optional hints.
...
run_resolver "$OUT" "$ERR" \
  ...
  scripts/widget.sh || rc=$?
[ "$rc" = 2 ] || fail "missing declared coverage must fail visibly"
[ ! -s "$OUT" ] || fail "invalid declarations must not return partial coverage"
grep -q 'no-such-sensor.sh' "$ERR" || fail "missing declaration must be identified"
run_resolver "$OUT" "$ERR" \
  --declared "tests/scripts/test_gadget.sh,tests/scripts/test_gadget.sh" scripts/widget.sh
grep -qx 'tests/scripts/test_gadget.sh' "$OUT" \
...
run_resolver "$OUT" "$ERR" --declared tests/scripts/validation/helpers/test_helper.sh \
  tests/scripts/validation/helper.sh || rc=$?
[ "$rc" = 2 ] || fail "declared helper must fail rather than silently lose coverage"
...
for shared in scripts/lib/trace-lib.sh scripts/trace/trace-lib.sh \
  ...
  run_resolver "$OUT" "$ERR" --declared tests/scripts/validation/test_widget.sh "$shared"
  [ "$(cat "$OUT")" = tests/scripts/validation/test_widget.sh ] \
    || fail "relocated shared authority must remain scoped: ${shared}"
done
...
[ "$rc" = 2 ] || fail "failed sensor discovery must stop feature verification"
...
printf 'PASS: affected-sensors resolver honors strict scoped selection\n'
