...
# Every fixture is an old summary without the optional experiment and
# same-class projections. Missing fields remain visible coverage gaps.
jq -e '
  all(.by_version[];
    ...
    and .same_class_failures.coverage.measured_inputs == 0
    and .same_class_failures.coverage.total_relevant_inputs == .runs)
' "$SCORECARD" >/dev/null 2>&1 \
  || fail "missing optional projections must stay null coverage gaps, never fabricated zero measurements"
