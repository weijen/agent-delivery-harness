...
# Historical feature_start spans remain accepted, but the section must state
# that they are no longer required for passes:true features.
...
if [ -f "$DOCTRINE" ]; then
  ...
  if [ -z "$handback_section" ]; then
    ...
    for token in 'feature_start' 'passes:true' 'historical'; do
      printf '%s' "$handback_section" | grep -qi -- "$token" \
        || fail "harness.instructions.md handoff section must mention '${token}' for retirement tolerance"
    done
    printf '%s' "$handback_section" | grep -Eqi '(not|required no longer|no longer required)' \
      || fail "harness.instructions.md must state feature_start is not required"
  fi
else
